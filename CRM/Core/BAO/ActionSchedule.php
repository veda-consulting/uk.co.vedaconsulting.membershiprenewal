...
";
   ...
   * @throws CRM_Core_Exception
   */
  public static function sendMailings($mappingID, $now, $params = array()) {
    ...
    while ($actionSchedule->fetch()) {
      $query = CRM_Core_BAO_ActionSchedule::prepareMailingQuery($mapping, $actionSchedule, $params);
  ...
  public static function processQueue($now = NULL, $params = array()) {
    ...
    foreach ($mappings as $mappingID => $mapping) {
      ...
      // RS Veda: Send $params array to sendMailings function, for options such as limit
      CRM_Core_BAO_ActionSchedule::sendMailings($mappingID, $now, $params);
    }
   ...
   * @return string
   */
  protected static function prepareMailingQuery($mapping, $actionSchedule, $params = array()) {
    ...
    // RS Veda: Add limit to query if set in scheduled job settings
  ...
  protected static function sendReminderEmail($tokenRow, $schedule, $toContactID) {
    ...
    //print_r ($tokenRow->context['actionSearchResult']);exit;
    ...
    // RS - Add activity id to scheduled reminder params
    if ($tokenRow->context['actionSearchResult']->entity_table == 'civicrm_activity') {
      $mailParams['activity_id'] = $tokenRow->context['actionSearchResult']->entity_id;
    }
